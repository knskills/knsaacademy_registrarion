...
(function () {
    ...
    function scrollToBottom(id) {
        ...
        var simpleBar = element.querySelector(
            "#chat-conversation .simplebar-content-wrapper"
        );
        if (!simpleBar) {
            console.log(
                `SimpleBar element not found within element with id "${id}"`
            );
            return;
        }

        var chatConversationList = element.getElementsByClassName(
            "chat-conversation-list"
        )[0];
        var offsetHeight = chatConversationList
            ? chatConversationList.scrollHeight - window.innerHeight + 250
            : 0;
